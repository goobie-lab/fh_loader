// fh_loader.cpp : Defines the entry point for the console application.
//
...
struct sparse_handle_t * sparse_open(FILE * fd, uint64_t current_start_sector, uint64_t sector_size_in_bytes)
{
	...
	return_sparse_handle->start_sector = (unsigned long long*) malloc(sparse_header.total_chunks*sizeof (unsigned long long));
	return_sparse_handle->bytes_remaining = (unsigned long long*)malloc(sparse_header.total_chunks*sizeof (unsigned long long));
	return_sparse_handle->group_file_offset = (long long*)malloc(sparse_header.total_chunks*sizeof (long int));
	...
	*/
	...
	for (i = 1; i < (SIZE_T)argc; i++) // skip 0 since argv[0] is fh_loader.exe
	{
		...
		else if (strncmp(MyOpt, "flattenbuildto=", MAX(strlen(MyOpt), strlen("flattenbuildto="))) == 0)
		{
			...
			if (mkdir(flattenbuildvariant) == -1)
				dbg(LOG_INFO, "Already exists");
			else
				dbg(LOG_INFO, "Created successfully");
